          ...
          <Dialog open={menuOpen} onOpenChange={setMenuOpen}>
            ...
            <DialogContent className="top-0 left-0 h-svh max-h-none w-[min(20rem,calc(100%-3rem))] max-w-none translate-x-0 translate-y-0 content-start gap-6 rounded-none border-y-0 border-l-0 px-4 pt-7 pb-[max(env(safe-area-inset-bottom),1.5rem)] sm:max-w-none" onCloseAutoFocus={(event) => {
              ...
              <nav aria-label={t("navigation")} className="grid gap-2">
                {navigation.map(({ section: item, icon: Icon }) => <button key={item} type="button" aria-current={section === item ? "page" : undefined} onClick={() => { onSectionChange(item); setMenuOpen(false); }} className={`flex min-h-12 items-center gap-3 rounded-xl px-4 text-left text-sm font-medium focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring ${section === item ? "bg-foreground text-background" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}><Icon className="size-4 shrink-0" aria-hidden="true" />{t(`nav.${item}`)}</button>)}
              </nav>
            </DialogContent>
          </Dialog>
        </div>}
