...
function PointsOverview({ member, locale }: { member: MemberSummary; locale: Locale }) {
  ...
  return (
    <div className="grid min-w-0 grid-cols-[minmax(0,.75fr)_minmax(0,1.25fr)] items-center gap-4 rounded-[1.4rem] border border-border bg-white/70 p-4 md:flex md:flex-col md:items-stretch md:justify-between md:p-6 lg:p-7">
      <div className="@container min-w-0">
        ...
        <p className="mt-2 flex min-w-0 flex-wrap items-baseline gap-x-2 gap-y-1 font-medium leading-none tracking-[-0.055em] tabular-nums">
          <span className="min-w-0 text-[clamp(1.5rem,17cqi,3.1rem)] [overflow-wrap:anywhere]">{formatNumber(member.points, locale)}</span>
          <span className="text-sm font-normal tracking-normal text-muted-foreground">{t("points")}</span>
        </p>
      </div>
      <div className="min-w-0 border-l border-border pl-4 md:mt-7 md:border-t md:border-l-0 md:pt-5 md:pl-0">
