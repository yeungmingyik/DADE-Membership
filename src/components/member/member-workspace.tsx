    ...
    <Dialog>
      ...
      <DialogContent className="max-w-sm rounded-3xl p-7">
        ...
        <div className="space-y-1 text-center">
          ...
          <p className="font-mono text-sm tracking-wider text-muted-foreground">{member.number}</p>
        </div>
      </DialogContent>
    </Dialog>
  );
...
function MembershipCard({ member }: { member: MemberSummary }) {
  const t = useTranslations("Member");

  return (
    <div className="relative isolate flex aspect-[1.65] min-h-[204px] w-full min-w-0 flex-col justify-between overflow-hidden rounded-[1.3rem] border border-white/15 bg-[#292925] p-6 text-[#f8f5ed] shadow-[0_18px_36px_-20px_rgba(33,30,24,0.55)] sm:p-7" aria-label={t("tierMembership", { tier: t(member.tier) })}>
      <div aria-hidden="true" className="pointer-events-none absolute -right-24 -top-44 -z-10 size-[480px] rounded-full border border-white/[0.06] bg-[radial-gradient(ellipse_at_center,_rgba(199,183,148,0.14),_transparent_66%)]" />
      <div aria-hidden="true" className="pointer-events-none absolute -bottom-44 -right-40 -z-10 size-[460px] rounded-full border border-white/[0.08]" />
      <div aria-hidden="true" className="pointer-events-none absolute -bottom-36 -right-28 -z-10 size-[360px] rounded-full border border-white/[0.07]" />
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-medium uppercase tracking-[0.24em] text-stone-300">{t("membership")}</p>
          <p className="mt-1 text-[10px] uppercase tracking-[0.24em] text-stone-400">{member.number}</p>
        </div>
        <span className={cn("inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-[10px] font-medium uppercase tracking-[0.12em]", member.tier === "gold" ? "border-[#c4b185]/40 text-[#e6d6ad]" : "border-white/20 text-stone-200")}>
      ...
      <div className="flex items-end justify-between gap-4">
        <div className="min-w-0">
          <p className="mb-1.5 text-[9px] uppercase tracking-[0.2em] text-stone-400">{t("cardholder")}</p>
          <p className="truncate text-xl font-medium tracking-tight sm:text-2xl">{member.name}</p>
        </div>
        <CreditCard aria-hidden="true" strokeWidth={1} className="size-8 shrink-0 text-stone-400" />
      </div>
    </div>
  );
}
...
function PointsOverview({ member, locale }: { member: MemberSummary; locale: Locale }) {
  ...
  return (
    <div className="flex flex-col justify-between rounded-[1.4rem] border border-border bg-white/70 p-6 sm:p-7">
      <div>
        <p className="text-xs font-medium text-muted-foreground">{t("pointsBalance")}</p>
        <p className="mt-2 flex items-baseline gap-2 text-[3.1rem] font-medium leading-none tracking-[-0.055em] tabular-nums">
      ...
      <div className="mt-7 border-t border-border pt-5">
        <div className="mb-3 flex items-center justify-between gap-3 text-xs">
          ...
          <span className="font-medium tabular-nums">{formatNumber(member.visits, locale)}{member.nextTierVisits !== null && <span className="font-normal text-muted-foreground"> / {formatNumber(member.nextTierVisits, locale)}</span>}</span>
        </div>
        ...
        <p className="mt-3 text-xs text-muted-foreground">{member.nextTierVisits !== null ? t("visitsToNextTier", { count: Math.max(0, member.nextTierVisits - member.visits) }) : t("highestTier")}</p>
      </div>
    </div>
  );
}
...
function ActivityRow({ activity, locale }: { activity: Activity; locale: Locale }) {
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <button className={cn("flex min-h-[84px] w-full items-center gap-3.5 rounded-xl py-4 text-left transition-colors hover:bg-stone-100/70 sm:gap-4 sm:px-2 motion-reduce:transition-none", focusClasses)}>
          <span className="flex size-11 shrink-0 items-center justify-center rounded-full border border-stone-200/80 bg-[#f1eeea] text-[#403a36]"><ActivityIcon kind={activity.kind} /></span>
          <span className="min-w-0 flex-1">
            <span className="block truncate text-sm font-medium">{t(activity.kind)}</span>
            <span className="mt-1 block truncate text-xs text-muted-foreground">{activity.storeName[locale]} · {formatDate(activity.occurredAt, locale)}</span>
          </span>
          <span className="shrink-0 text-right">
            ...
            {activity.amountCents !== null && <span className="mt-1 block text-xs tabular-nums text-muted-foreground">{formatMoney(activity.amountCents, locale)}</span>}
          </span>
    ...
    <Dialog>
        ...
        <button className={cn("group w-full overflow-hidden rounded-2xl border border-border bg-white/80 text-left transition-shadow hover:shadow-sm motion-reduce:transition-none", focusClasses)}>
          ...
          <div className="p-4">
            ...
            <div className="mt-2 flex items-center justify-between gap-2">
              <p className="text-xs text-muted-foreground">{t("pointsValue", { count: reward.points })}</p>
              <ArrowUpRight aria-hidden="true" className="size-4 text-stone-500 transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5 motion-reduce:transform-none motion-reduce:transition-none" />
            </div>
          </div>
        </button>
      </DialogTrigger>
      <DialogContent className="max-w-sm rounded-3xl p-7">
        ...
        <div className="flex items-center justify-between gap-4">
          <p className="font-medium">{t("pointsValue", { count: reward.points })}</p>
          <p className="text-xs text-muted-foreground">{reward.stock > 0 ? t("stockCount", { count: reward.stock }) : t("outOfStock")}</p>
        </div>
      </DialogContent>
    </Dialog>
  );
...
function MemberNavigation({ locale, section }: { locale: Locale; section: MemberSection }) {
  ...
  return (
    <nav aria-label={t("navigation")} className="fixed inset-x-0 bottom-0 z-30 border-t border-stone-200 bg-[#fbfaf8]/95 px-3 pb-[max(env(safe-area-inset-bottom),0.5rem)] pt-2 backdrop-blur-md sm:bottom-5 sm:left-1/2 sm:right-auto sm:w-[430px] sm:-translate-x-1/2 sm:rounded-2xl sm:border sm:p-2 sm:shadow-[0_8px_35px_-15px_rgba(41,37,36,0.2)]">
      <div className="mx-auto grid max-w-md grid-cols-4 gap-1">
        {tabs.map(({ name, icon: Icon }) => <Link key={name} href={`/${locale}/member${name === "card" ? "" : `?view=${name}`}`} aria-current={section === name ? "page" : undefined} className={cn("flex min-h-[52px] flex-col items-center justify-center gap-1 rounded-xl px-2 py-2 text-[10px] font-medium transition-colors motion-reduce:transition-none", section === name ? "bg-[#eeebe5] text-foreground" : "text-stone-500 hover:bg-stone-100 hover:text-foreground", focusClasses)}><Icon aria-hidden="true" className="size-[19px]" strokeWidth={section === name ? 1.8 : 1.5} />{t(name)}</Link>)}
      </div>
    </nav>
  );
}
...
export function MemberWorkspace({ locale, data, section = "card" }: MemberWorkspaceProps) {
  ...
  return (
    <div className="mx-auto w-full max-w-5xl px-5 pb-32 pt-7 sm:px-8 sm:pt-10">
      <header className="mb-7 flex items-end justify-between gap-4 sm:mb-9">
        ...
        {section === "card" && <span className="mb-0.5 flex size-11 items-center justify-center rounded-full border border-border bg-white/70 text-sm font-medium text-stone-600" aria-hidden="true">{member.name.slice(0, 1).toLocaleUpperCase(locale)}</span>}
      </header>

      {section === "card" && <>
        <section className="grid grid-cols-1 gap-4 sm:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)] sm:gap-6" aria-label={t("card")}>
          <div className="min-w-0 space-y-4"><MembershipCard member={member} /><MemberCode member={member} /></div>
          <PointsOverview member={member} locale={locale} />
        </section>
        <div className="mt-9 grid grid-cols-1 gap-9 lg:mt-11 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,1fr)] lg:gap-10">
      ...
      {section === "rewards" && <>
        <div className="mb-6 flex items-center justify-between rounded-2xl border border-border bg-white/70 px-5 py-4"><span className="text-sm text-muted-foreground">{t("pointsBalance")}</span><span className="text-lg font-medium tracking-tight tabular-nums">{t("pointsValue", { count: member.points })}</span></div>
